...
describe(`${NAME}`, () => {
  it('isElementInEventPath', () => {
    ...
    // 不存在触发路径
    expect(isElementInEventPath(ele, { target: ele, ...event })).toBe(true)

    // 正常设置 composedPath
    expect(isElementInEventPath(ele, { composedPath: () => [container, ele], ...event })).toBe(true)

    // 正常设置 path
    expect(isElementInEventPath(ele, { path: [container, ele], ...event })).toBe(true)

    // 正常设置 deepPath
    expect(isElementInEventPath(ele, { deepPath: () => [container, ele], ...event })).toBe(true)
  ...
  it('getTargetEleInEventPath', () => {
    ...
    expect(
      getTargetEleInEventPath([container, ele], function (ele) {
