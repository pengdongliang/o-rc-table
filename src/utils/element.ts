 ...
 * @param popupParent
 */
export function calculatePointerRelative(
  event: React.MouseEvent<HTMLTableElement, MouseEvent> | MouseEvent,
...
function getPerfectY({ y, maxY, height, scrollTop, isBody, isPerfectY }) {
  if (isPerfectY && Math.max(y, 0) > Math.abs(maxY)) {
    // 看下上方空间是否足够
    const parentAvailableHeight = isBody ? y - scrollTop : y
